...
public class CashBookListActionBean extends BaseBean {
	...
	private StreamedContent getDownloadValue(List<CashBookDTO> dtoList) {
		try {
			...
			String currencyCode = criteria.getCurrencyType().equals(CurrencyType.HOMECURRENCY) ? "All Currencies" : criteria.getCurrency().getCurrencyCode();
			if (criteria.isHomeCurrencyConverted()) {
				currencyCode = currencyCode + " By Home Currency Converted";
			}
