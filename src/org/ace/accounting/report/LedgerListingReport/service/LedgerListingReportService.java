...
public class LedgerListingReportService extends BaseService implements ILedgerListingReportService {
	...
	private BigDecimal findCCOAOpeningBalance(ChartOfAccount coa, LedgerDto ledgerDto) throws SystemException {
		...
		if (currencyType.equals(CurrencyType.HOMECURRENCY)) {
			openingField = BusinessUtil
					.getMonthSRCStatusJPQLField(Integer.valueOf(BusinessUtil.getBudgetInfo(startDate, 3)) - 1);
		} else {
			if (ledgerDto.isHomeCurrencyConverted()) {
				...
				openingField = BusinessUtil
						.getMonthStatusJPQLField(Integer.valueOf(BusinessUtil.getBudgetInfo(startDate, 3)) - 1);
			}
		}
		...
		sf = new StringBuffer(
				"SELECT Sum(c." + openingField + ") FROM VwCcoa c WHERE c.coa=:coa AND c.budget=:budgetYear");
		// if (coa == null) {
	...
	public strictfp List<AccountLedgerDto> findAccLedgerListing(LedgerDto ledgerDto, List<ChartOfAccount> coaList) throws SystemException {
		...
		if (sDate > 1) {
			sf = new StringBuffer(
					"SELECT NEW org.ace.accounting.dto.AccountLedgerDto(t.ccoa.coa.id,t.ccoa.coa.acType, t.localAmount, t.localAmount, t.homeAmount, t.homeAmount,t.narration, t.currency.id, t.branch.id, t.tranType.tranCode) FROM TLF t WHERE t.reverse=0 AND t.paid=1 AND t.settlementDate BETWEEN "
			...
			if (allBranch && admin) { // For All Branch
				if (currencyType.equals(CurrencyType.HOMECURRENCY)) {
					...
					debitCreditList = accLedgerList.stream()
							.map(p -> new AccountLedgerDto(p.getCoaId(),p.getAcType(), p.getHomeDebit(), p.getHomeCredit(), p.getNarration(), p.getSettlementDate())).collect(Collectors.toList());
					...
					if (homeCurrencyConverted) { // By Home Currency Converted
						...
						//// Debit, Credit
						debitCreditList = accLedgerList.stream().filter(p -> p.getCurrencyId().equals(currency.getId()))
								.map(p -> new AccountLedgerDto(p.getCoaId(),p.getAcType(), p.getDebit(), p.getCredit(), p.getNarration(), p.getSettlementDate())).collect(Collectors.toList());
					}
				}
			} else { // Not All Branch
				if (currencyType.equals(CurrencyType.HOMECURRENCY)) {
...
//			for (AccountLedgerDto dto : debitCreditList) {
//				if (dto.getAcType().equals(AccountType.A) || dto.getAcType().equals(AccountType.E)) {
//					dblBalance = dblBalance.subtract(dto.getCredit()).setScale(2,RoundingMode.HALF_UP);
//					dblBalance = dblBalance.add(dto.getDebit()).setScale(2,RoundingMode.HALF_UP);
//				} else if (dto.getAcType().equals(AccountType.I) || dto.getAcType().equals(AccountType.L)) {
//					dblBalance = dblBalance.add(dto.getCredit()).setScale(2,RoundingMode.HALF_UP);
//					dblBalance = dblBalance.subtract(dto.getDebit()).setScale(2,RoundingMode.HALF_UP);
//				}
//				dto.setDblBalance(dblBalance);
//				dto.setSrNo(srNo);
//			}
		...
		sf = new StringBuffer(
				"SELECT NEW org.ace.accounting.dto.AccountLedgerDto(t.ccoa.coa.id,t.ccoa.coa.acType, t.localAmount, t.localAmount, t.homeAmount, t.homeAmount, t.narration, t.settlementDate, t.currency.id, t.branch.id, t.tranType.tranCode) FROM TLF t WHERE t.reverse=0 AND t.paid=1 AND t.settlementDate BETWEEN "
						+ ":startDate" + " AND " + ":endDate" + " AND t.ccoa.coa=:coa ORDER BY t.settlementDate");
		...
		if (allBranch && admin) { // For All Branch
			if (currencyType.equals(CurrencyType.HOMECURRENCY)) {
				debitCreditList = accLedgerList.stream().map(p -> new AccountLedgerDto(p.getCoaId(),p.getAcType(), p.getHomeDebit(), p.getHomeCredit(), p.getNarration(), p.getSettlementDate()))
						.collect(Collectors.toList());
			} else {
...
//			}
		...
		srNo += 1;
		debitCreditList1.add(new AccountLedgerDto(coa.getId(),coa.getAcType(), getTotalDebit(debitCreditList), getTotalCredit(debitCreditList), "Closing Balance", dblBalance, endDate, srNo));
		}
	...
	public List<AccountLedgerDto> findDebitCreditByStartDate(StringBuffer sf, ChartOfAccount coa, Date sDate)
	...
	@Transactional(propagation = Propagation.REQUIRED)
	public List<AccountLedgerDto> findDebitCreditBySDateAndEDate(StringBuffer sf, ChartOfAccount coa, Date startDate,
	...
	@Transactional(propagation = Propagation.REQUIRED)
	public List<GenLedgerDto> findGenLedgerListing(LedgerDto ledgerDto, ChartOfAccount targetCoa)
			throws SystemException {
		...
		try {
			...
			for (ChartOfAccount coa : coaList) {
				...
				List<GLDBDto> ledgerList = ledgerListingReportDAO.findGeneralLedgerList(ledgerDto, coa, startDate,
						endDate);
...
//						dblBalance = new BigDecimal(dblBalance.doubleValue() - dto.getDebit().doubleValue());
					...
					detailList.add(new GenLedgerDetail(dto.getDebit(), dto.getCredit(), dblBalance,
							dto.getSettlementDate(), srNo));
				}
				srNo += 1;
	...
	private BigDecimal getUnpostedBalance(LedgerDto ledgerDto, ChartOfAccount coa) {
		...
		Date startDate = DateUtils
				.formatStringToDate("1" + "-" + (cal.get(Calendar.MONTH) + 1) + "-" + (cal.get(Calendar.YEAR)));
